...
public class StudentListPanel extends JPanel {
    ...
    private void loadStudentData(String className, String groupName) {
        ...
        // 加载学生成绩数据
        Map<String, String> studentScores = new HashMap<>();  // key: 班级,学生名 value: 成绩
        ...
        if (studentFile.exists()) {
            try (BufferedReader reader = new BufferedReader(new FileReader(studentFile))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parts.length >= 4) {
                        if (className == null || 
                            (className.equals(parts[0]) && groupName == null) || 
                            (className.equals(parts[0]) && groupName.equals(parts[3]))) {
                            
                            int[] counts = attendanceData.getOrDefault(parts[0] + "," + parts[2], new int[2]);
                            String score = studentScores.getOrDefault(parts[0] + "," + parts[2], "0");
